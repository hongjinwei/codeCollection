...
#--------------------
REQ_NUM = 1     #number of rquests per thread 
THREAD_NUM = 1   #number of threads
LOOP_MODE = None #True or False
#--------------------
...
class BidRequestGenerator(object):
    ...
    def _GenerateId(self,length):
        ...
        #return "".join(id)
	str = "".join(id)
	return "jd63oxyfyhcz4hxgcoku8v3yjunbeewp"
...
def make_case():
    ...
    #data = string.ljust("hello",300,"x")
    str_data = bidrequest.SerializeToString()
    #file = open('baidutest.txt','wb')
    #file.write(str_data)
    #file.close()
    #return data
    return str_data
...
def process_send(i):
    #connection = httplib.HTTPConnection('172.18.0.143','80')
    for cnt in range(REQ_NUM):
        print "------:" , cnt
        req = make_case()
	#connection = httplib.HTTPConnection('bidder.bilintechnology.net','80')
        connection = httplib.HTTPConnection('172.18.0.143','80')
        connection.request('POST','/besbid', req ,{'Content-type':'application/octet-stream'})
...
def do_send():
    ...
    for i in range(THREAD_NUM):
        p = Process(target=process_send,args=(i,))
        #p.start()
        pList.append(p)
